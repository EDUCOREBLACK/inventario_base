...
<body>
                        ...
                        <ol class="breadcrumb">
                            <li class="">
                               <a href="../"> <i class="fa fa-dashboard"></i> Inicio </a>
                                 <li class="active">
                                <i class="fa fa-dashboard"></i> Gestion Sala 2
                            </li>
 ...
 <div class="row">
                    ...
                    <div class="col-lg-12">
                        <h1 class="page-header">
                       Exportar Informacion de Inventario en EXCEL Sala 2
...
if ($result = mysqli_query($link, "SELECT * FROM sala2 where nombre='Disponible'")) {
    $row_cnt = mysqli_num_rows($result);
}

if ($result1 = mysqli_query($link, "SELECT *  FROM  rack  where sala='2'")) {
    $row_cnt1 = mysqli_num_rows($result1);
}

 if ($result2 = mysqli_query($link, "SELECT *  FROM  sala2  ")) {
 ...
 if ($result3 = mysqli_query($link, "SELECT *  FROM  sala2 where  razon_social<> '1' and razon_social <> 'disponible' ")) {
 ...
 if ($resultSERVER  = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'SERVER ' ")) {
    $row_SERVER  = mysqli_num_rows($resultSERVER );
}

 if ($resultSWITCH = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'SWITCH ' ")) {
    $row_SWITCH = mysqli_num_rows($resultSWITCH);
}

 if ($resultROUTER = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'ROUTER' ")) {
    $row_ROUTER = mysqli_num_rows($resultROUTER);
}

 if ($resultTRANSCEIVER = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'TRANSCEIVER' ")) {
    $row_TRANSCEIVER = mysqli_num_rows($resultTRANSCEIVER);
}

 if ($resultSTORAGE = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'STORAGE' ")) {
    $row_STORAGE = mysqli_num_rows($resultSTORAGE);
}


 if ($resultFIREWALL = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'FIREWALL' ")) {
    $row_FIREWALL = mysqli_num_rows($resultFIREWALL);
}

 if ($resultLIBRERIA = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'LIBRERIA' ")) {
    $row_LIBRERIA = mysqli_num_rows($resultLIBRERIA);
}

 if ($resultMODEM = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'MODEM' ")) {
    $row_MODEM = mysqli_num_rows($resultMODEM);
}

 if ($resultUNIDAD_CINTA = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'UNIDAD DE CINTA' ")) {
    $row_UNIDAD_CINTA= mysqli_num_rows($resultUNIDAD_CINTA);
}

 if ($resultDTU = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'DTU' ")) {
    $row_DTU = mysqli_num_rows($resultDTU);
}

 if ($resultGATEWAY = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'GATEWAY' ")) {
    $row_GATEWAY = mysqli_num_rows($resultGATEWAY);
}


 if ($resultLIBRERIA_RESPALDO = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'LIBRERIA RESPALDO ")) {
    $row_LIBRERIA_RESPALDO = mysqli_num_rows($resultLIBRERIA_RESPALDO);
}

 if ($resultMONITOR = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'MONITOR' ")) {
    $row_MONITOR = mysqli_num_rows($resultMONITOR);
}

 if ($resultNTU = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'NTU' ")) {
    $row_NTU = mysqli_num_rows($resultNTU);
}

 if ($resultCONTROLADORA = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'CONTROLADORA' ")) {
    $row_CONTROLADORA = mysqli_num_rows($resultCONTROLADORA);
}

 if ($resultCABECERA = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'CABECERA DE F.O' ")) {
    $row_CABECERA = mysqli_num_rows($resultCABECERA);
}

 if ($resultCONSOLA = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'CONSOLA' ")) {
    $row_CONSOLA = mysqli_num_rows($resultCONSOLA);
}

 if ($resultMULTIPLEXOR = mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'MULTIPLEXOR' ")) {
    $row_MULTIPLEXOR = mysqli_num_rows($resultMULTIPLEXOR);
}

 if ($resultPATCHPANEL= mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'PATCH PANEL' ")) {
    $row_PATCHPANEL = mysqli_num_rows($resultPATCHPANEL);
}
if ($resultENCLOSURE= mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'ENCLOSURE' ")) {
    $row_ENCLOSURE = mysqli_num_rows($resultENCLOSURE);
}
if ($resultBALANCEADOR= mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'BALANCEADOR' ")) {
    $row_BALANCEADOR = mysqli_num_rows($resultBALANCEADOR);
}
if ($resultIPS_IDS= mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'IPS/IDS' ")) {
    $row_IPS_IDS = mysqli_num_rows($resultIPS_IDS);
}
if ($resultCONMUTADOR= mysqli_query($link, "SELECT *  FROM  sala2 where tipo_equipo= 'CONMUTADOR' ")) {
...
var chart = Highcharts.chart('container', {
    ...
    title: {
        text: 'Informacion de equipos en sala 2'
    },
